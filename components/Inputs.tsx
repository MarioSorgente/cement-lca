...
/** Tiny inline tooltip (no external CSS needed) */
function Info({ text }: { text: string }) {
  ...
  return (
    ...
    >
      <span
        style={{
          width: 16, height: 16, borderRadius: 9999, background: '#e2e8f0', color: '#334155',
      ...
      {open && (
        <span
          ...
          style={{
            ...
            fontSize: 12, whiteSpace: 'nowrap', boxShadow: '0 6px 20px rgba(0,0,0,0.15)', zIndex: 20
          }}
        >
          {text}
        </span>
...
export default function Inputs({ state, setState }: { state: InputsState; setState: (s: InputsState) => void }) {
  ...
  return (
    <div className="card" style={{ marginBottom: 16 }}>
      ...
      <div className="grid" style={{ gridTemplateColumns: '1fr 1fr' }}>
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => setState({ ...state, exposureClass: e.target.value })}
          >
      ...
      {/* Row 2: Element volume + Binder dosage group */}
      <div className="grid" style={{ gridTemplateColumns: '1fr 1fr', marginTop: 16 }}>
        ...
        {/* Binder dosage group: Global dosage + Dosage mode */}
        <div>
          <label className="label" style={{ display: 'flex', alignItems: 'center' }}>
            Binder dosage
            <Info text="Cement content per m³. Choose a single global dosage or allow per-cement overrides in the table." />
          </label>

          <div
            style={{
              border: '1px solid #e2e8f0', borderRadius: 10, background: '#f9fafb',
          ...
          >
            <div>
              ...
              <input
                ...
                disabled={dosageDisabled}
                style={dosageDisabled ? { opacity: 0.6, cursor: 'not-allowed' } : undefined}
              />
      ...
      {/* Row 3: Transport emissions group (A4) */}
      <div style={{ marginTop: 16 }}>
        <label className="label" style={{ display: 'flex', alignItems: 'center' }}>
          Transport emissions (A4)
          <Info text="A4 = transport from plant to site. Calculated as volume × distance × transport factor (kg CO₂ per m³·km). Tick to include A4 in totals." />
        </label>
