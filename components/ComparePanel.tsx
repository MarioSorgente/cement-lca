...
export default function ComparePanel({
  open, onClose,
  rows, selectedIds, onToggle,
  onDosageChange, catalog, baselineId
}: Props) {
    ...
    <>
      ...
      <aside className={`cmp-drawer ${open ? 'open' : ''}`} aria-hidden={!open}>
        <div className="cmp-drawer-header">
          ...
          <button className="button" style={{ padding:'4px 8px', borderRadius: 10 }} onClick={onClose} aria-label="Close compare panel">✕</button>
        </div>

        <div className="cmp-drawer-body">
          {selectedRows.length === 0 && (
            <div className="small" style={{ marginBottom: 8 }}>
              No items selected — open “Add items” below to pick some.
            </div>
          )}
                ...
                <div style={{ display: 'flex', alignItems:'flex-start', justifyContent:'space-between', gap: 12 }}>
                  <div>
                    <div className="cmp-drawer-title">
                      ...
                      {isBest && <span className="pill pill-deepgreen" style={{ marginLeft: 8 }}>Most sustainable</span>}
                    </div>
                    ...
                    <div style={{ marginTop: 8 }}>
                      ...
                      <input
                        ...
                        onChange={(e) => onDosageChange?.(r.cement.id, Math.max(0, Number(e.target.value || 0)))}
                        disabled={!onDosageChange}
                        aria-label={`Dosage for ${r.cement.cement_type}`}
                      />
                      <div className="small" style={{ marginTop: 6 }}>
                        <button className="cmp-fab-clear" onClick={() => onToggle(r.cement.id)} aria-label={`Remove ${r.cement.cement_type} from compare`}>
                          Remove
                        </button>
                      </div>
                    </div>
                  ...
                  >
                    <div className="small" style={{ color:'#64748b', marginBottom: 4 }}>
                      Total CO₂ element <span className="small" style={{ marginLeft: 4, color:'#94a3b8' }}>(kg)</span>
                    </div>
                ...
                <div className="cmp-bars">
                  <div className="cmp-bars-track">
                    <div className="cmp-bars-total" style={{ width: '100%' }}>
                      <div className="cmp-bars-a1a3" style={{ width: `${r.totalElement === 0 ? 0 : (r.co2ePerM3_A1A3 / r.totalElement) * 100}%` }} />
                      <div className="cmp-bars-a4"   style={{ width: `${r.totalElement === 0 ? 0 : (r.a4Transport / r.totalElement) * 100}%` }} />
                    </div>
                  </div>
          ...
          {/* Collapsible add items */}
          <div style={{ marginTop: 8 }}>
            <button
              className="button"
              onClick={() => setAddOpen(o => !o)}
              aria-expanded={addOpen}
              aria-controls="cmp-add"
            ...
            {addOpen && (
              <div id="cmp-add" style={{ marginTop: 8 }}>
                <input
                  className="input"
                ...
                <div style={{ marginTop: 8, display: 'grid', gap: 6, maxHeight: 260, overflow: 'auto' }}>
                  ...
                  {list.length === 0 && (
                    <div className="small" style={{ color:'#64748b' }}>No matches.</div>
                  )}
                </div>
              </div>
            )}
